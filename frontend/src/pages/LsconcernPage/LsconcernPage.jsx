      ...
      <div className={`${styles.blogContent} ${isActive ? styles.hiddenContent : ''}`}>
        ...
        <div className={styles.cardBottomFixed}>
          ...
          <div className={styles.cardMetaRow}>
            ...
            <span className={styles.categoryBadge}>Legal & Social</span>
          </div>
        </div>
      </div>
          ...
          <div className={styles.bottomFixed}>
            <div className={styles.leftContent}>
              ...
              <div className={styles.authorDateContainer}>
                <span className={styles.authorName}>Legal & Social</span>
                ...
                onClick={(e) => {
                  ...
                  if (navigator.share) {
                    navigator.share({
                      ...
                      url: `/blog/lsconcern/${post.id}`
                    });
                  } else {
                    navigator.clipboard.writeText(`${window.location.origin}/blog/lsconcern/${post.id}`);
...
const LsConcernPage = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      ...
      try {
        const res = await fetch('/api/blogs/lsconcern');
        ...
        setError(`Failed to load Legal & Social posts: ${err.message}`);
        setPosts([]);
      } finally {
        setLoading(false);
      }
  ...
  const handlePostClick = (post) => {
    navigate(`/blog/lsconcern/${post.id}`);
  };
          ...
          <div className={styles.blogGrid}>
            {loading ? (
              <div className={styles.loadingState}>
                ...
                <p>Loading Legal & Social posts...</p>
              </div>
            ) : error ? (
              ...
              <div className={styles.emptyState}>
                <h3>No Legal & Social Posts Available</h3>
                <p>Check back soon for new legal insights and social commentary.</p>
              </div>
            ) : (
              posts.map((post) => (
                <LegalSocialCard
